...
public class Server implements Runnable {
	...
	public Server(IServerConfiguration configuration,
			IRequestProcessor processor) throws IOException {
		this.processor = processor;
		this.configuration = configuration;
		selector = initSelector();
		executor = Executors.newFixedThreadPool(configuration
				.getThreadPoolSize());
		System.out.println("Set up server socket channel [PORT=" + configuration.getPort() + "]");
	}
	...
	private Selector initSelector() throws IOException {
		...
		serverChannel.socket().bind(new InetSocketAddress(configuration.getPort()));
